...
import {JSTickFormatter} from "@bokehjs/models/formatters"
...
describe("Slider", () => {
  ...
  it("should support TickFormatter format", () => {
    const format = new JSTickFormatter({code: "return (tick/1000).toFixed(0) + 'k'"})
...
describe("DateSlider", () => {
  ...
  it("should support TickFormatter format", () => {
    const format = new JSTickFormatter({code: "return Math.floor(1970 + tick/(1000*60*60*24*365)).toFixed(0)"})
